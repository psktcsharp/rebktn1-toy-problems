...
var Range = function(start, end, step) {
  this.storage = {"start":start,"end":end,"step": step }
  this.outPut = []

  if(this.storage["end"] === undefined){
      this.storage["end"] === this.storage["start"]
  }
  var stepper = this.storage["step"];
  if(this.storage["step"]  === undefined){
      this.storage["step"] === 1
      stepper = 1
  }

  if(this.storage["start"] < this.storage["end"] ){
  for(var i = this.storage["start"]; i<= this.storage["end"]; i+=stepper){
      this.outPut.push(i)
  }
  }
  else{
